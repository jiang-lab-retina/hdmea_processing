"""
Step 4: Radial Center Analysis for Blocker Comparison
=====================================================
Searches for optimal radial centers on hexbin data for
before/after/delta conditions.

Reuses logic from radial_center_analysis.py.
"""
...
# =====================================================================
# Core functions (from radial_center_analysis.py)
# =====================================================================
...
def search_radial_center(bx, by, vals):
    """Two-pass grid search + scipy refinement."""
...
def main():
    hex_files = {
        "before": OUTPUT_DIR / "hexbin_before_all.parquet",
        "after": OUTPUT_DIR / "hexbin_after_all.parquet",
        "delta": OUTPUT_DIR / "hexbin_delta_all.parquet",
    }

    all_results = []

    for condition, fpath in hex_files.items():
        if not fpath.exists():
            logger.warning(f"  Missing: {fpath}")
            continue

        logger.info(f"\n=== {condition} radial center search ===")
        ...
        for fi, feat in enumerate(features):
            sub = df_hex[df_hex["feature"] == feat]
            if len(sub) < MIN_BINS:
                continue
    ...
    for condition in ["before", "after", "delta"]:
        ...
        if len(sub) > 0:
            out_path = OUTPUT_DIR / f"radial_center_{condition}.parquet"
...
def _write_summary(df_all):
    lines = []
    lines.append("# Radial Center Analysis: Blocker Comparison\n")
    ...
    before_raw = df_all[
        (df_all["condition"] == "before") & (df_all["data_type"] == "raw_mean")
    ].set_index("feature")
    after_raw = df_all[
        (df_all["condition"] == "after") & (df_all["data_type"] == "raw_mean")
    ].set_index("feature")
    if len(before_raw) > 0 and len(after_raw) > 0:
        common = before_raw.index.intersection(after_raw.index)
        if len(common) > 0:
            lines.append("\n## Before vs After Radial Center Shifts\n")
            lines.append("| Feature | Before Cx | Before Cy | After Cx | After Cy | Shift (um) | Before |r| | After |r| |")
            lines.append("|---------|-----------|-----------|----------|----------|------------|-----------|----------|")
            for feat in common:
                br = before_raw.loc[feat]
                ar = after_raw.loc[feat]
                shift = np.sqrt(
                    (br["best_center_x"] - ar["best_center_x"]) ** 2 +
                    (br["best_center_y"] - ar["best_center_y"]) ** 2
                )
                lines.append(
                    f"| {feat} | {br['best_center_x']:.0f} | {br['best_center_y']:.0f} "
                    f"| {ar['best_center_x']:.0f} | {ar['best_center_y']:.0f} "
                    f"| {shift:.0f} | {abs(br['best_r']):.4f} | {abs(ar['best_r']):.4f} |"
                )
            lines.append("")
