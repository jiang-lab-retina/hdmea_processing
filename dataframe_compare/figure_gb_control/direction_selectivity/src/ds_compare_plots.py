"""
DS Compare Hexbin + GAM Heatmaps (4 conditions)
================================================
For each direction-selectivity feature and amplitude ratio, generates
multi-panel comparison figures across four conditions: before (control),
STR, PTX, STR_PTX.
...
FIG_DIR = FIG_DIR_BASE / "direction_selectivity" / "compare_all"
...
def _generate_all_cells_figures(df, available_features, trace_dict,
    ...
    """One figure per DS feature: 5 rows x 4 cols gridspec."""
    logger.info("\n=== Phase 1: All-cells compare figures ===")
...
def main():
    ...
    if not input_path.exists():
        logger.error("Input not found: %s\nRun prepare_ds_compare_data.py first.", input_path)
        return
    ...
    if not available_features:
        logger.error("No DS features found -- nothing to plot.")
        return
    ...
    if metrics_rows:
        met_path = OUTPUT_DIR / "ds_compare_metrics.parquet"
