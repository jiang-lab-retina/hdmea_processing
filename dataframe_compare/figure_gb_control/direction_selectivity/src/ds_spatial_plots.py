"""
DS Spatial Hexbin + GAM Heatmaps
================================
Creates hexbin heatmaps (raw + GAM smoothed) for each direction-selectivity
feature and amplitude ratio, at two scopes:
...
FIG_DIR = FIG_DIR_BASE / "direction_selectivity"
...
def main():
    input_path = OUTPUT_DIR / "combined_ds_control.parquet"
    if not input_path.exists():
        logger.error(
            "Input not found: %s\nRun prepare_ds_data.py first.", input_path
        )
        return
    ...
    available_features = [f for f in DS_PLOT_FEATURES if f in df.columns]
    logger.info("  Available DS features: %d", len(available_features))
    if not available_features:
        logger.error("No DS features found in the parquet -- nothing to plot.")
        return
