"""
Freq-Step Spatial Hexbin + GAM Heatmaps
========================================
Creates hexbin heatmaps (raw + GAM smoothed) for each scalar frequency-step
...
FIG_DIR = FIG_DIR_BASE / "freq_step"
...
def main():
    input_path = OUTPUT_DIR / "combined_fs_control.parquet"
    if not input_path.exists():
        logger.error(
            "Input not found: %s\nRun prepare_fs_data.py first.", input_path
        )
        return
    ...
    if not available_features:
        logger.error("No FS features found in the parquet -- nothing to plot.")
        return
    ...
    logger.info("  Generated %d feature maps", len(m))
    ...
    for si, subtype in enumerate(subtypes):
        ...
        n_cells = len(sub_df)
        if n_cells < 5:
            logger.info("  [%d/%d] %s -- skipped (n=%d)",
                        si + 1, len(subtypes), subtype, n_cells)
            continue
        ...
        logger.info("  [%d/%d] %s: %d cells, %d features",
                     si + 1, len(subtypes), subtype, n_cells, len(m))
    ...
    if all_cells_rows:
        out = OUTPUT_DIR / "fs_hexbin_all_cells.parquet"
        pd.DataFrame(all_cells_rows).to_parquet(out, index=False)
        logger.info("  %s: %d rows", out.name, len(all_cells_rows))

    if per_sub_rows:
        out = OUTPUT_DIR / "fs_hexbin_per_subtype.parquet"
        pd.DataFrame(per_sub_rows).to_parquet(out, index=False)
        logger.info("  %s: %d rows", out.name, len(per_sub_rows))

    if metrics_rows:
        out = OUTPUT_DIR / "fs_spatial_metrics.parquet"
        pd.DataFrame(metrics_rows).to_parquet(out, index=False)
        logger.info("  %s: %d rows", out.name, len(metrics_rows))
