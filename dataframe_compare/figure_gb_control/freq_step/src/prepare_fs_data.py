"""
Prepare Combined Freq-Step Control Dataset
===========================================
Loads before-blocker frequency-step feature columns from all 3 experiments
(_ptx_str, _ptx, _str), strips the ``before_`` prefix, computes amplitude
ratio features, drops rows without valid spatial coordinates, and saves a
single combined parquet for freq-step spatial analysis.
"""
...
ALL_FS_FEATURES = FS_SCALAR_FEATURES + [r[0] for r in AMP_RATIO_FEATURES]
...
def main():
    ...
    for exp, parquet_path in SOURCE_PARQUETS.items():
        ...
        before_cols = [
            f"before_{feat}" for feat in FS_SCALAR_FEATURES
        ...
        frames.append(sub)
        logger.info("  Kept %d FS columns, %d rows", len(before_cols), len(sub))
    ...
    available = [f for f in ALL_FS_FEATURES if f in combined.columns]
    logger.info("Available FS features: %d / %d", len(available), len(ALL_FS_FEATURES))
    for f in available:
    ...
    out_path = OUTPUT_DIR / "combined_fs_control.parquet"
