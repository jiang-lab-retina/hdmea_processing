"""
Freq-Step Compare Hexbin + GAM Heatmaps (4 conditions)
=======================================================
For each scalar freq-step feature, generates multi-panel comparison figures
across four conditions: before (control), STR, PTX, STR_PTX.

Two scopes:
  1. **all_cells** -- 5 rows x 4 cols GridSpec.
     Rows 0-3: raw hexbin (cols 0-1) + GAM (cols 2-3) per condition.
     Row 4: freq-step averaged trace, one subplot per condition.
  ...
  2. **per_subtype** -- 5 rows x 4 cols GridSpec per cluster.
     ...
     Row 4: freq-step averaged trace per condition (within the subtype).

Freq-step traces are loaded directly from source parquets (not stored in the
combined FS parquet) following the pattern in
...
AMP_RATIO_NAMES = [
    "freq_step_1hz_amp_ratio_05hz",
    "freq_step_2hz_amp_ratio_05hz",
    "freq_step_4hz_amp_ratio_05hz",
    "freq_step_10hz_amp_ratio_05hz",
]

ALL_FS_FEATURES = FS_SCALAR_FEATURES + AMP_RATIO_NAMES

FREQ_TRACE_NAME = "freq_step_5st_3x"

FIG_DIR = FIG_DIR_BASE / "freq_step" / "compare_all"
...
def _load_freq_traces():
    """Load both before_ and after_ freq-step traces from all 3 source parquets.

    Returns a DataFrame with columns:
        before_dataset_id, subtype, source_experiment,
        before_{FREQ_TRACE_NAME}, after_{FREQ_TRACE_NAME}
    """
    before_col = f"before_{FREQ_TRACE_NAME}"
    after_col = f"after_{FREQ_TRACE_NAME}"
    frames = []
...
def _plot_freq_trace(ax, trace_info, cond_label):
    """Plot mean freq-step trace +/- std shading."""
...
def _generate_all_cells_figures(df, available_features, trace_dict,
    ...
    """One figure per FS feature: 5 rows x 4 cols gridspec."""
    logger.info("\n=== Phase 1: All-cells compare figures ===")
    ...
    for fi, feat in enumerate(available_features):
        ...
        for ci, cond in enumerate(CONDITIONS):
            ...
            _plot_freq_trace(ax_step, t_info, cond)
...
def main():
    ...
    input_path = OUTPUT_DIR / "combined_fs_compare.parquet"
    if not input_path.exists():
        logger.error("Input not found: %s\nRun prepare_fs_compare_data.py first.", input_path)
        return
    ...
    available_features = [f for f in ALL_FS_FEATURES if f in df.columns]
    logger.info("  Available FS features: %d", len(available_features))
    if not available_features:
        logger.error("No FS features found -- nothing to plot.")
        return
    ...
    logger.info("\n=== Loading freq-step traces ===")
    trace_df = _load_freq_traces()
    if len(trace_df) > 0:
    ...
    if metrics_rows:
        met_path = OUTPUT_DIR / "fs_compare_metrics.parquet"
