#!/usr/bin/env python
"""
Batch update RF geometry + DSGC direction sectioning for existing HDF5 files.

Loads HDF5 files from export_all_steps/, runs:
1. RF geometry extraction (with LNL fitting)
2. DSGC direction sectioning

and saves results to export_dsgc_updated/.

Usage:
    python update_dsgc_sta.py
    python update_dsgc_sta.py --start 0 --end 10
    python update_dsgc_sta.py --overwrite
    python update_dsgc_sta.py --input path/to/input --output path/to/output
"""
...
# Import RF geometry session functions
...
OUTPUT_DIR = Path(__file__).parent.parent / "export_dsgc_sta_updated"
...
def process_single_file(
    ...
    """
    Process a single HDF5 file through RF geometry + DSGC steps.
    ...
    try:
        ...
        # =====================================================================
        # Step 1: RF Geometry Extraction (with LNL fitting)
        # =====================================================================
        logger.info(f"  Preparing stimulus data for LNL fitting...")
        cover_range = (-60, 0)
        frame_rate = 15.0
        ...
        if movie_array is not None and spike_frames_dict is not None:
            logger.info(f"  Stimulus movie loaded: {movie_array.shape}")
            logger.info(f"  Spike frames available for {len(spike_frames_dict)} units")
        else:
            logger.info(f"  LNL fitting will be skipped (missing stimulus or spike data)")
            spike_frames_dict = None  # Ensure it's None for downstream check
        
        # Clear RF geometry step from completed_steps so it runs again
        session.completed_steps.discard("extract_rf_geometry")
        
        # Run RF geometry extraction
        ...
        # Count units with RF geometry and LNL fit
        rf_count = sum(
            1 for ud in session.units.values()
            if ud.get('features', {}).get('sta_perfect_dense_noise_15x15_15hz_r42_3min', {}).get('sta_geometry') is not None
        )
        lnl_count = sum(
            1 for ud in session.units.values()
            if ud.get('features', {}).get('sta_perfect_dense_noise_15x15_15hz_r42_3min', {}).get('sta_geometry', {}).get('lnl') is not None
        )
        logger.info(f"  RF geometry: {rf_count} units, LNL fit: {lnl_count} units")
        
        # =====================================================================
        # Step 2: DSGC Direction Sectioning
        # =====================================================================
        ...
        return True, f"success ({session.unit_count} units, {rf_count} RF, {lnl_count} LNL)"
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch update RF geometry (with LNL) + DSGC for existing HDF5 files."
    )
    parser.add_argument(
        "--input", "-i",
        type=Path,
    ...
    logger.info("=" * 60)
    logger.info("RF Geometry + DSGC Update Batch Processing")
    logger.info("=" * 60)
