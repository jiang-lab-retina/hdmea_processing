...
# Generates dot plots and hexbin heatmaps for spatial feature distributions.
# Input: labeled_dataframe.parquet from classification pipeline.
...
INPUT_PARQUET = os.path.join(
    ...
    "results", "labeled_dataframe.parquet",
)

FIG_DIR = os.path.join(SPATIAL_DIR, "figures")
...
EXCLUDE_PATTERNS = ["transformed_", "cartesian_", "polar_", "freq_step_"]
...
def plot_hexbin_all_cells(df, feature, save_dir):
    ...
    if c_std == 0:
        print(f"  [skip] {feature}: constant value")
        return
    vmin = c_mean - 2 * c_std
    vmax = c_mean + 2 * c_std
...
def plot_hexbin_per_subtype(df, feature, valid_subtypes, save_dir):
    ...
    c_std = float(np.std(c_all))
    if c_std == 0:
        print(f"  [skip subtype] {feature}: constant value")
        return
    vmin = c_mean - 2 * c_std
    vmax = c_mean + 2 * c_std
    ...
    for idx in range(used_slots, total_slots):
        ...
        # Remap: first n*2 slots are used, rest hidden
        axes[row][col].set_visible(False)
    # Also hide trailing empty pairs
