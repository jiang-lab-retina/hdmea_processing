...
# %% [markdown]
# # Spatial Distribution Plots (Improved Coordinates - v6 robust ONH)
# Same plots as spatial_plots_legacy.py but using improved_tx / improved_ty
# from improve_onh_v6 (robust ONH, legacy transform, no rotation).
...
INPUT_PARQUET = os.path.join(SCRIPT_DIR, "labeled_dataframe_improved_coords.parquet")
...
print(f"  After dropping NaN coords: {df_raw.shape}")
...
# ==========================================================================
# Plot helpers (identical to spatial_plots_legacy.py)
# ==========================================================================
...
def plot_hexbin_all_cells(df, feature, save_dir):
    ...
    plot_hexbin_single(ax_raw, x, y, c, GRIDSIZE_ALL, MINCNT_ALL, CMAP, vmin, vmax)
    ax_raw.set_title("Raw mean", fontsize=11)
    fit_gam_predict_hexbin(ax_gam, x, y, c, GRIDSIZE_ALL, MINCNT_ALL, CMAP, vmin, vmax,
    ...
    fig.savefig(os.path.join(save_dir, f"Hexbin_{feature}.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_hexbin_per_subtype(df, feature, valid_subtypes, save_dir):
    ...
    for idx, stype in enumerate(valid_subtypes):
        ...
        plot_hexbin_single(ax_raw, sx, sy, sc, GRIDSIZE_SUB, MINCNT_SUB, CMAP, vmin, vmax)
    ...
    fig.savefig(os.path.join(save_dir, f"Hexbin_{feature}_subtypes.png"), dpi=200, bbox_inches="tight")
    plt.close(fig)
