...
class GaussianMixtureGPU:
    ...
    def _e_step(self, X: torch.Tensor) -> torch.Tensor:
        ...
        # log N(x | mu, sigma^2) = -0.5 * (d*log(2pi) + sum(log(var)) + sum((x-mu)^2/var))
        
        log_probs = torch.zeros(n_samples, k, device=self.device)
        
        for j in range(k):
            diff = X - self._means[j]  # (n_samples, n_features)
            var = self._vars[j]  # (n_features,)
            
            # Mahalanobis distance for diagonal covariance
            mahal = (diff ** 2 / var).sum(dim=1)  # (n_samples,)
    ...
    def _m_step(self, X: torch.Tensor, resp: torch.Tensor) -> None:
        ...
        nk = resp.sum(dim=0) + 1e-10  # (n_components,)
    ...
    def _compute_log_likelihood(self, X: torch.Tensor) -> float:
        ...
        # Log-likelihood is sum of log-sum-exp over components
    ...
    def bic(self, X: np.ndarray) -> float:
        ...
        For diagonal covariance GMM:
        n_params = k * (1 + 2*d) - 1
          - k-1 for weights (sum to 1 constraint)
          - k*d for means
